...
export async function listStrategies(agentAlias) {
    try {
...
export async function toggleStrategy(strategyId, agentAlias, enable) {
    try {
        // First get the strategy
        const getResponse = await axios.get(`${DASHBOARD_API}/api/strategies?agent=${agentAlias}`);
        const strategy = getResponse.data.find((s) => s.id === strategyId);
        if (!strategy) {
...
export async function deleteStrategy(strategyId, agentAlias) {
    try {
        await axios.delete(`${DASHBOARD_API}/api/strategies?id=${strategyId}&agent=${agentAlias}`);
        return { success: true };
    }
    catch (error) {
        return {
...
export async function testStrategy(strategyId) {
    try {
...
export function formatStrategyList(strategies) {

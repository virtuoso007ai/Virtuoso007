...
export const DEFAULT_DEGEN_LEADERBOARD_API = "https://degen.virtuals.io/api/leaderboard";
function esc(s) {
...
export async function fetchLeaderboardAll(seasonId) {
...
export async function fetchLeaderboardTop(seasonId, limit) {
    const base = leaderboardBaseUrl();
...
export function indexByWallet(rows) {
    const m = new Map();
    for (const r of rows) {
...
export function formatLbRowLine(entry) {
...
export function buildLeaderboardHtml(opts) {
    ...
    const note = previewOnly
        ...
        : "";
    return (`<b>Degen leaderboard</b> · <i>${esc(seasonName)}</i> · kayıt <code>${total}</code>\n` +

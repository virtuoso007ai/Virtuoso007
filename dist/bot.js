...
import { buildLeaderboardHtml, defaultSeasonId, fetchLeaderboardAll, fetchLeaderboardTop, indexByWallet, normalizeWallet, } from "./leaderboard.js";
import { HELP } from "./help.js";
import { createStrategy, listStrategies, toggleStrategy, deleteStrategy, testStrategy, formatStrategyList, } from "./strategy.js";
/** /open@BotName arg1 arg2 → [arg1, arg2] */
function commandRest(ctx) {
...
async function cancelLimitsOnPair(client, pair, wallet) {
...
function isAuthorized(chatId, allowed) {
    if (!chatId)
        return false;
    if (allowed.size === 0) {
        console.warn("[bot] ALLOWED_CHAT_IDS tanımlı değil — tüm sohbetlere açık (üretimde mutlaka kısıtla!)");
        return true;
    }
    return allowed.has(String(chatId));
}
function requireAgent(agents, alias) {
    if (!alias)
        return null;
 ...
 */
async function replyChunkedHtml(ctx, html) {
    const max = 3500;
...
export function registerBot(bot, agents) {
    ...
    bot.command("agents", async (ctx) => {
        const lines = [...agents.values()].map((a) => `• ${a.alias}${a.label ? ` — ${a.label}` : ""}`);
    ...
    bot.command(["positions", "poz"], async (ctx) => {
        ...
        if (!wallet) {
            await ctx.reply("<i>Cüzdan bulunamadı.</i> <code>apiKey</code> / <code>/acp/me</code> veya <code>walletAddress</code> kontrol et.", { parse_mode: "HTML" });
            return;
        }
        ...
        try {
            ...
            await ctx.reply(`<b>Hata</b>\n<pre>${escHtml(errText(e).slice(0, 3500))}</pre>`, { parse_mode: "HTML" });
        }
    });
    bot.command(["balance", "bakiye", "account"], async (ctx) => {
        ...
        if (sub.toLowerCase() === "all") {
            ...
            for (const a of [...agents.values()].sort((x, y) => x.alias.localeCompare(y.alias))) {
                const w = await resolveWalletAddress(a);
                if (!w) {
                    blocks.push(`${a.alias} — cüzdan alınamadı (walletAddress veya /acp/me)`);
                    continue;
                }
                try {
        ...
        const wallet = await resolveWalletAddress(agent);
        if (!wallet) {
            await ctx.reply("Cüzdan bulunamadı. apiKey ile /acp/me veya AGENTS_JSON’da walletAddress.");
            return;
        }
        await ctx.reply("Çekiliyor…");
        try {
    ...
    bot.command(["leaderboard", "lb"], async (ctx) => {
        ...
        const collectMatches = async (byW) => {
            const out = [];
            for (const a of [...agents.values()].sort((x, y) => x.alias.localeCompare(y.alias))) {
                const w = await resolveWalletAddress(a);
                if (!w) {
        ...
        try {
            if (mode === "top") {
                ...
                await replyChunkedHtml(ctx, buildLeaderboardHtml({
                    seasonName,
                    total,
                    matches,
                    topRows: rows,
            ...
            await ctx.reply(`<b>Leaderboard hatası</b>\n<pre>${escHtml(errText(e).slice(0, 3500))}</pre>`, { parse_mode: "HTML" });
        }
    });
    bot.command("open", async (ctx) => {
        ...
        if (!pair || (side !== "long" && side !== "short") || !sizeRaw) {
            await ctx.reply("Kullanım: /open <alias> <PAIR> <long|short> <size> [kaldıraç] [SL] [TP] [orderType] [limitPrice]\nÖrnek: /open raichu ETH long 50 10\nLimit: /open raichu ETH long 50 10 2000 2200 limit 2100");
            return;
        }
    ...
    bot.command("cancel", async (ctx) => {
        ...
        if (!pair) {
            await ctx.reply("Kullanım: /cancel <alias> <PAIR> [oid]\n" +
        ...
        await ctx.reply(`Limit iptal: ${agent.alias} ${pair}${oidStr ? ` oid=${oidStr}` : " (tümü)"}…`);
        try {
            const client = createAcpClient(agent.apiKey);
            if (oidStr) {
    ...
    bot.command("modify", async (ctx) => {
        ...
        const pair = pairRaw?.toUpperCase();
        const skipToken = (s) => s == null || /^[-_]$|^(skip|yok|none)$/i.test(String(s).trim());
        const agent = requireAgent(agents, alias);
        if (!agent) {
            await ctx.reply("Geçersiz alias.");
            return;
    ...
    bot.command(["openmulti", "openm"], async (ctx) => {
        ...
        if (!aliasesRaw || !pairRaw || !sideRaw || !sizeRaw) {
            await ctx.reply("Kullanım: /openmulti <alias1,alias2,...> <PAIR> <long|short> <size> [kaldıraç] [SL] [TP] [orderType] [limitPrice]\n" +
    ...
    bot.command("openall", async (ctx) => {
        ...
        if (!pairRaw || !sideRaw || !sizeRaw) {
            await ctx.reply("Kullanım: /openall <PAIR> <long|short> <size> [kaldıraç] [SL] [TP] [orderType] [limitPrice]\n" +
    ...
    bot.command(["closemulti", "closem"], async (ctx) => {
        ...
        if (!aliasesRaw || !pairRaw) {
            await ctx.reply("Kullanım: /closemulti <alias1,alias2,...> <PAIR>\n" +
    ...
    bot.command(["modifymulti", "modifym"], async (ctx) => {
        ...
        if (!aliasesRaw || !pairRaw) {
            await ctx.reply("Kullanım: /modifymulti <alias1,alias2,...> <PAIR> [SL] [TP] [leverage]\n" +
    ...
    bot.command("modifyall", async (ctx) => {
        ...
        if (!pairRaw) {
            await ctx.reply("Kullanım: /modifyall <PAIR> [SL] [TP] [leverage]\n" +
            ...
            switch (subcommand) {
                case "create": {
                    ...
                    if (result.success && result.strategy) {
                        await replyChunkedHtml(ctx, `✅ <b>Strategy Created</b>\n\n` +
                ...
                case "test": {
                    ...
                    if (result.success && result.signal) {
                        ...
                        await replyChunkedHtml(ctx, `📊 <b>Strategy Test Result</b>\n\n` +
